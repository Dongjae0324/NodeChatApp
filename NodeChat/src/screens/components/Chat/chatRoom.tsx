...
const ChatRoom = ({route}) => {
  ...
  useEffect(() => {
    webSocket.current = io(`http://localhost:3000/chat`, {
      auth: {title: title, user: user},
    });
  ...
  const sendMessage = async () => {
    try {
      ...
      const response = await axios.post(
        `http://localhost:3000/room/${title}/chat`,
        data,
      );
      ...
      <FlatList
        ...
        renderItem={({item}) =>
          item.type === 'join' || item.type === 'exit' ? (
            <Text
              style={{alignSelf: 'center', color: 'grey', paddingVertical: 10}}>
              {item.chat}
            </Text>
          ) : item.user === user ? (
...
const styles = StyleSheet.create({
  ...
  myChat: {
    ...
    marginBottom: 2,
    fontSize: 15,
    ...Platform.select({ios: {paddingVertical: 5}}),
  },
  otherChatUser: {
